...
class Example
{
    ...
    public function mapContactOntoRemotePerson(int $id, OsdiPerson $remotePerson = null): OsdiPerson
    {
        ...
        if (is_null($remotePerson)) $remotePerson = new OsdiPerson();
    ...
    public function mapRemotePersonOntoContact(OsdiPerson $remotePerson, int $contactId = null): AbstractAction
    {
        ...
        if ($rpAddress) {
            list($stateId, $countryId) = $this->getStateAndCountryIdsFromOsdiAddress($rpAddress);
    ...
    private function civiApi4GetSingleContactById($id): array
    {
        $result = Contact::get(FALSE)
            ...
            ->addJoin('Phone AS phone', FALSE, NULL, ['phone.phone_type_id:name', '=', '"Mobile"'])
            ->addJoin('Address AS address', FALSE, NULL, ['address.is_primary', '=', 1])
            ->setSelect([
                            'id',
                            'first_name',
                            'last_name',
    ...
    private function getStateAndCountryIdsFromOsdiAddress(array $osdiAddress): array
    {
        ...
        if (isset($osdiAddress['country'])) {
            ...
            if ($idFromAbbrev !== false) $countryId = $idFromAbbrev;
        }
        if (!($stateAbbrev = $osdiAddress['region'])) return [null, $countryId];
    ...
    {
        if (!($rpLanguages = $remotePerson->get('languages_spoken'))) return null;
        $rpLanguage = $rpLanguages[0];
        $map = ['en' => 'en_US', 'es' => 'es_MX'];
        if (in_array($rpLanguage, $map)) return $map[$rpLanguage];
        $civiLangs = \CRM_Contact_BAO_Contact::buildOptions('preferred_language', 'create');
        foreach ($civiLangs as $isoCode => $label) {
            if ($rpLanguage === substr($isoCode, 0, 2)) return $isoCode;
        }
        return null;
    }
}